...
typedef boost::mpl::list<int8_t, uint16_t, int32_t> test_types; // fail for, long long,  uint64_t, etc.
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test05_size_num_blocks, T, test_types ) {
  ...
  if (bits1.num_blocks()== ceil(static_cast<double>(33) /
...
//       ceil(static_cast<double>(nb_bits) / static_cast<double>(sizeof(T) * 8.)));
//   // std::cout << "8*sizeof(T)=" << 8 * sizeof(T) << std::endl;
//   // std::cout << "static_cast<size_type>( ceil(static_cast<double>(" << nb_bits
//   //           << ") / static_cast<double>(sizeof(T)*8.))=" << number_of_block
//   //           << std::endl;
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test09_operators_masks, T, test_types) {
  ...
  mbitset1.set(32, 1); mbitset1.set(63, 1); mbitset1.set(64, 1);

  bfl::bitset<T> mbitset2(67);
  mbitset2[0] =8; mbitset2(64) = 1;  mbitset2(62) = 1;
...
//   mbitset.set(3, 1);
//   mbitset.set(63, 1);
//   // std::cout << mbitset << std::endl << "n=" << mbitset.count_ones() << std::endl;
//   // std::cout << "76543210 76543210 76543210 76543210 76543210 76543210"
//   //           << std::endl;
//   // std::cout << mbitset << std::endl << std::endl << std::endl;
//   // if ( bits.max_size()==m_bits.max_size()* bits_per_block )
...
// BOOST_AUTO_TEST_CASE_TEMPLATE(test10_operator_integer_and_bool, T, test_types) {
//   bfl::bitset<T> bitset1(67);
//   bfl::bitset<T> bitset2 = bitset1.bit_mask(33);
