...
// different options depending on compile or runtime needs
//  (fixed size array?)

// I would have used the boost version of the three possibilities
// boost::dynamic_bitset<int> bits;
// boost/dynamic_bitset/dynamic_bitset.hpp
// or
// std::bitset<2> bits;
// in /usr/include/c++/7/bitset
// or
// Boost.Compute \c dynamic_bitset
// boost/compute/container/dynamic_bitset.hpp
// I decided to copy, paste and adapt
...
public:
  ...
  explicit
  bitset(const Allocator& alloc);
  
  explicit
  bitset(size_type num_bits, unsigned long value = 0,
  ...
  /// Returns the number of blocks to store the bits in the dynamic bitset. // num_integer
  ...
  block_type bit_mask(size_type pos) noexcept { return block_type(1) << bit_index(pos); }
  ...
  block_type &operator[](const size_type n) { return *(m_bits.begin() + n); }
...
operator&(const bitset<Block, Allocator>& b1,
...
template <typename Block, typename Allocator>
bitset<Block, Allocator>::
bitset(size_type num_bits, unsigned long value, const Allocator& alloc)
...
template <typename Block, typename Allocator>
bitset<Block, Allocator>& bitset<Block, Allocator>::
operator=(const bitset<Block, Allocator>& b)
{
    m_bits = b.m_bits;
...
template <typename Block, typename Allocator>
inline bitset<Block, Allocator>::
bitset(bitset<Block, Allocator>&& b)
...
template <typename Block, typename Allocator>
inline bitset<Block, Allocator>& bitset<Block, Allocator>::
operator=(bitset<Block, Allocator>&& b)
{
...
template <typename Block, typename Allocator>
bitset<Block, Allocator>&
bitset<Block, Allocator>::operator&=(const bitset& rhs)
{
...
template <typename Block, typename Allocator>
bitset<Block, Allocator>
operator&(const bitset<Block, Allocator>& x,
...
//   //     // wrong for int8_t
//   //     size_type counter = 0;
//   //     for (typename std::vector<block_type>::const_iterator it = m_bits.begin(); it < m_bits.end(); ++it)
//   //       counter += __builtin_popcount(*it); // <--- builtin GCC for int32_t
//   //     return counter;
//   //   }
// }
