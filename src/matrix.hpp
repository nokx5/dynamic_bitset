...
template <typename T> matrix<T>::matrix() : row_{}, col_{}, elem_{} {}
template <typename T> matrix<T>::~matrix() {}
template <typename T>
matrix<T>::matrix(std::size_t row, std::size_t col)
    : row_(row), col_(col), elem_(row * col) {}
template <typename T>
matrix<T>::matrix(const matrix<T> &mat)
    : row_(mat.row_), col_(mat.col_), elem_(mat.elem_) {}
...
template <typename T> std::ostream &matrix<T>::print(std::ostream &out) const {
  out << " matrix, i x j = dim :  " << row_ << " x " << col_ << " = "
      << row_ * col_ << std::endl
      << std::endl;
...
template <typename T> std::size_t matrix<T>::num_rows() const { return row_; }
template <typename T> std::size_t matrix<T>::num_cols() const { return col_; }
template <typename T> std::size_t matrix<T>::num_bit_row(std::size_t row) {
...
template <typename T> std::size_t matrix<T>::num_bit_col(std::size_t col) {
...
matrix<T>::operator&=(const matrix<T>& rhs)
{
  if (row_ != rhs.row_ || col_ != rhs.col_)
    throw std::logic_error("|-->  error in matrix<T>::operator&=, matrix have different dimensions");
